...
@ParametersAreNonnullByDefault public final class JsonDiff {
	private static final MessageBundle BUNDLE = MessageBundles
			.getBundle(JsonPatchMessages.class);
	private static final ObjectMapper MAPPER = JacksonUtils.newMapper();

	private static final Equivalence<JsonNode> EQUIVALENCE = JsonNumEquals
			.getInstance();
	...
	public static JsonPatch asJsonPatch(final JsonNode source,
		...
		return asJsonPatch(source, target, false);
	}
	 ...
	 */
	public static JsonPatch asJsonPatch(final JsonNode source,
			final JsonNode target, boolean includeOpTest) {
		BUNDLE.checkNotNull(source, "common.nullArgument");
		BUNDLE.checkNotNull(target, "common.nullArgument");
		final Map<JsonPointer, JsonNode> unchanged = getUnchangedValues(source,
				target);
		final DiffProcessor processor =
				includeOpTest ? new TestAndSetDiffProcessor(unchanged) :
						new DiffProcessor(unchanged);
	 ...
	 */
	public static JsonNode asJson(final JsonNode source,
			final JsonNode target) {
	...
	public static JsonNode asJson(final JsonNode source, final JsonNode target,
			boolean includeOpTest) {
		final String s;
		try {
			s = MAPPER.writeValueAsString(
					asJsonPatch(source, target, includeOpTest));
	...
	private static void generateObjectDiffs(final DiffProcessor processor,
		...
		final Set<String> firstFields = Sets
				.newTreeSet(Sets.newHashSet(source.fieldNames()));
		final Set<String> secondFields = Sets
				.newTreeSet(Sets.newHashSet(target.fieldNames()));
	...
	private static void computeUnchanged(final Map<JsonPointer, JsonNode> ret,
			final JsonPointer pointer, final JsonNode first,
